class Table:
    ...
    # busca linear
    def linear_search(self, key, position):
        if position > self.size():
            ...
            if self.keys[position] == key:
                ...
                return self.linear_search(key, position + 1)

    # busca binaria
    def binary_search(self, key, short, high):
        while high >= short:
            middle = (short + high) // 2
            ...
            if kick == key:
                return middle
            if kick > key:
                return self.binary_search(key, short, middle - 1)
            if kick < key:
    ...
    def type_search(self, key, option):
        # busca binaria
        if option == 0:
            return self.binary_search(key, 1, self.size())
        # busca linear
        elif option == 1:
            return self.linear_search(key, 1)
        else:
            return self.binary_search(key, 1, self.size())
